...
6. 主畫面按住 `Space` 錄音，放開或 2.2 秒自動停止；Codec2 700C 壓縮完成後排入發送，剛好是一個 LoRa 封包。接收端只有在語音格式驗證通過後才自動播放。
