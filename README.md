...
Cardputer Mesh Terminal 是針對 **M5Stack Cardputer Adv + Cap LoRa-1262/GNSS** 的實驗性離網通訊韌體。第一版核心是選單式罐頭訊息、英數文字、GNSS Beacon/隊友雷達與軌跡，並內建最多 2.2 秒的 Codec2 PTT 語音及受控 Mesh 中繼。
...
3. 輸入相同的 4 位群組 PIN。PIN 會決定 group id、頻道與 sync word。PIN 以明碼顯示，方便當場核對按錯的位數——4 位 PIN 的安全目標本來就只有群組辨識與基本內容遮蔽，遮成 `*` 擋不住任何實際威脅。
...
9. 主畫面按 `-`／`=` 以 10% 為一級調整喇叭音量，範圍 0–100%，開機預設 50%。這兩顆是鍵盤上直接印著的鍵，不需要 `Shift`（`_` 與 `+` 同樣有效）。
...
訊息與語音存在 flash 上獨立的 4.9 MB `logfs` 分割區，與韌體、NVS 和 SD 上的軌跡完全分開。可用空間不足時只會自動刪除**最舊的語音**，文字紀錄不會因為空間而被刪除——軌跡與 GPS 位置在 SD 上，也不受影響。

分割表在此版本變更，升級必須整片抹除後燒錄（`deploy\flash-dev.cmd -EraseAll`），否則裝置上不存在 `logfs`，歷史會降級為只存在記憶體。
