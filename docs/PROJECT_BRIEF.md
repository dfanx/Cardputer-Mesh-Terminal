...
- **預期成果**：單一可燒錄韌體提供群組配對、基本內容遮蔽與完整性、去中心化最多兩跳中繼、文字、GNSS Beacon、軌跡/雷達，以及最多三秒的 Codec2 PTT 語音。
...
- `T` 直接進入英數自由輸入；`M` 開啟可視化罐頭訊息選單並支援數字快速發送；MicroSD `message.txt` 自訂短語。
- GNSS 解析、10 分鐘 Beacon、當日軌跡、隊友距離/方位/高差。
- Space PTT、3 秒限制、Codec2 1300 壓縮、單跳語音與播放；音訊不可用時不阻塞主功能。
