...
- **Consequences**：分割表改變，燒錄必須整片抹除更新；`deploy\flash-dev.ps1` 因此把完整抹除從選用旗標改為每次都執行，舊裝置升級會清掉既有 NVS 以外的內容（使用者代號、配對狀態）。放棄 OTA 能力（本專案本來就只用 USB 燒錄）。app 分割由 3.3 MB 縮到 3 MB，目前用量 1.19 MB，仍有兩倍以上餘裕。
- **Revisit trigger**：需要 OTA；語音長度或保存策略改變使 4.9 MB 不足；或改用外部 flash/SD 作為主要歷史儲存。
- **Note（2026-08-15，見 ADR-011）**：本決策記錄當時 SD 仍保存 `/tracks/*.csv`；ADR-011 移除了全軌跡記錄，SD 目前只保存 `/message.txt`。這裡的內容按決策當下的事實保留，不回頭改寫。
...
## ADR-010 — 群組 PIN 隨使用者代號一併持久化

- **Date**：2026-08-15
- **Status**：accepted
- **Context / constraints**：使用者代號會存進 NVS、下次開機自動帶入，PIN 卻每次都要求重打。分割表變更後（ADR-008）升級必須整片抹除，抹除會清掉 NVS，使用者代號與配對狀態因此常態性地一起重設——重打代號還好，重打 4 位 PIN 是每次升級或重開機都要重複的操作摩擦。FIRMWARE.md 原本的存放設計明文寫「不保存明文 PIN」（ADR-003 當時的立場）。
- **Options considered**：維持不存、每次要求輸入；只存 PIN 的雜湊供比對但仍要求輸入明碼（無法回填，等於沒解決問題）；比照使用者代號直接存明碼 PIN 到 NVS。
- **Decision**：`DeviceState` 新增 `pin()`/`setPin()`，比照 `userId()`/`setUserId()` 存進同一個 NVS namespace。配對成功時寫入；開機時預先帶入配對畫面，使用者要嘛直接 Enter 沿用、要嘛 Backspace 清掉重打。
- **Rationale**：ADR-003 已經確立「4 位 PIN 的安全目標只有群組辨識與基本內容遮蔽，不構成強安全」，且更早的另一項決策已經把 PIN 輸入畫面改成明碼顯示，理由是「遮成 `*` 擋不住任何實際威脅，卻讓使用者看不出按錯哪一位」。這兩個既有決策已經確立了「拿到這台裝置＝看得到 PIN」的實際威脅模型；PIN 出現在畫面上與出現在 NVS 裡，暴露對象是同一群人（持有裝置的人）。既然明碼顯示都已經接受，NVS 多存一份不構成新的威脅面，卻能把「開機/升級後重新配對」的操作成本降到跟使用者代號一樣低。
- **Consequences**：FIRMWARE.md「不保存明文 PIN」的敘述需更新為僅適用於 message_id/sequence 等欄位；`DeviceState` 的 NVS namespace 多一個 4 bytes 字串鍵。裝置遺失時，攻擊者不需要再猜 PIN——但 ADR-003 早已承認 PIN 空間只有 10,000 種組合，物理持有裝置的攻擊者原本就能直接讀畫面上的明碼，這裡不是新增的洩漏管道。
- **Revisit trigger**：PIN 的安全目標改變（例如要支援多群組輪替、需要防止「撿到裝置＝直接冒充成員」）；或改用生物辨識/裝置鎖等額外閘門保護 NVS 存取。

## ADR-011 — 移除全軌跡記錄，主畫面改為隊友相對位置雷達

- **Date**：2026-08-15
- **Status**：accepted
- **Context / constraints**：v1 主畫面左側方框畫的是把當次開機以來的 GNSS fix 連成折線的「軌跡圖」，SD 上同時寫 `/tracks/YYYYMMDD.csv`。實機使用回報：消費級 GNSS 模組在原地不動時的座標飄移量，在 145×77 像素的小螢幕折線圖上經常畫成明顯的鋸齒或亂繞，這張圖對「隊友現在在哪個方向、多遠」這個真正會被查閱的問題沒有幫助，形同雜訊。
- **Options considered**：維持折線圖，加低通濾波降飄移（濾波參數難調，且改善有限，飄移量本身就在數十公尺量級）；折線圖與雷達圖並存（兩者都要塞進同一個小方框，資訊密度過高、失去兩者原本的可讀性）；整個移除折線圖，改成以本機為中心的隊友相對位置雷達。
- **Decision**：拿掉 `StorageService::maybeAppendTrack()`、SD `/tracks/*.csv` 寫入與 `UiHomeModel::track`；主畫面方框改畫雷達：中心點代表本機，每位隊友依 `relativePosition()` 算出的方位與距離畫成一個點，比例尺依目前最遠的隊友自動縮放並在右下角標示。沒有任何隊友有可用相對位置時顯示提示文字，不畫空雷達。另外在罐頭訊息選單加一項「分享目前位置」，觸發時立即送一次 Beacon（不等 10 分鐘排程）並重設下一次排程時間，讓對方的雷達點盡快更新；隊友之間原有的 10 分鐘 Beacon 週期本身不變。
- **Rationale**：雷達直接回答「隊友在哪」，折線圖回答「我走過哪裡」——後者在單機、小螢幕、消費級 GNSS 的組合下沒有實用的導航價值，卻要吃 SD I/O 與畫面空間。相對位置本來就已經算好用在隊友清單上，雷達只是把同一份資料換一種畫法，不需要新的資料來源。
- **Consequences**：SD 上不再產生 `/tracks/*.csv`；已裝置上舊的軌跡檔不會被自動清除，但韌體不再讀寫。`docs/PROJECT_BRIEF.md` 的範圍敘述需同步移除「當日軌跡」。這是產品範圍的縮減，不是純技術重構。
- **Revisit trigger**：使用者需要事後匯出行進路徑（例如上呈搜救單位）；或換裝精度更高的 GNSS 模組後折線圖的飄移問題不再成立。
